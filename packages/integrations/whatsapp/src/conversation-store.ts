...
class WhatsAppConversationStore {
  ...
  addMessage(
    ...
    console.log(
      `[WhatsAppConversationStore] Added ${role} message for user ${userId}, account ${accountId}`,
    );
  }
  ...
  clearConversation(userId: string, accountId: string): void {
    ...
    console.log(
      `[WhatsAppConversationStore] Cleared conversation for user ${userId}, account ${accountId}`,
    );
  }
