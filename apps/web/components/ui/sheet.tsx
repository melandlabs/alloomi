...
const SheetOverlay = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof SheetPrimitive.Overlay> & {
    /** Use absolute when rendering inside a specified container, otherwise use fixed */
    positionInside?: boolean;
  }
>(({ className, positionInside, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      ...
      positionInside ? "absolute inset-0" : "fixed inset-0",
      className,
    )}
    {...props}
    ref={ref}
...
interface SheetContentProps
  ...
  /**
   * When a mount container is specified, Sheet will render inside that container with absolute positioning (used for sidebars inside dialogs, etc.)
   */
...
>(({ side = "right", className, children, container, ...props }, ref) => {
  const positionInside = Boolean(container);
  return (
    <SheetPortal container={container ?? undefined}>
      <SheetOverlay positionInside={positionInside} />
      <SheetPrimitive.Content
        ref={ref}
        className={cn(
          sheetVariants({
            side,
            positionMode: positionInside ? "inside" : "fixed",
          }),
          className,
        )}
        {...props}
