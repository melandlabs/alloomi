...
export async function sendReplyByBotId({
	...
	try {
		...
		if (bot.adapter === "slack") {
			...
			const { FacebookMessengerAdapter } = await import(
				"@alloomi/integrations/facebook-messenger"
			);
			...
			const { InstagramAdapter } = await import(
				"@alloomi/integrations/instagram"
			);
			...
			const { DingTalkAdapter } = await import(
				"@alloomi/integrations/dingtalk"
			);
