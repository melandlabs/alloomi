  ...
  for (let i = cssMatches.length - 1; i >= 0; i--) {
    ...
    if (isRemote) {
      ...
      try {
        ...
        // When Google Fonts fails (e.g., in Tauri webview with no internet access),
        // replace with a system font stack fallback
        if (
          cssPath.includes("fonts.googleapis.com") ||
          cssPath.includes("fonts.gstatic.com")
        ) {
          processedHtml =
            processedHtml.slice(0, matchIndex) +
            processedHtml.slice(matchIndex + fullTag.length);
